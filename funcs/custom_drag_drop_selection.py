####
#### Class that handles custom filtering by user
####
...
class custom_category_handle(object):


	def __init__(self,platform,source_data,column_header,path_file_icons):
	
		self.source_data = source_data
		self.source_data_columns = column_header
		...
		self.operator_var.set('OR')
		self.icon_path = path_file_icons
		self.annotate_category = tk.BooleanVar()
		self.annotate_category.set(False)
		...
		if self.platform == 'MAC':
			...
			self.keep_icon = ImageTk.PhotoImage(Image.open(os.path.join(self.icon_path,'check_icon.png')))
			self.trash_icon = ImageTk.PhotoImage(Image.open(os.path.join(self.icon_path,'delete_icon.png')))
			self.dustbin_icon = ImageTk.PhotoImage(Image.open(os.path.join(self.icon_path,'dustbin.png')))
			self.car_icon = ImageTk.PhotoImage(Image.open(os.path.join(self.icon_path,'car_add.png')))
			
		else:
			self.keep_icon =  tk.PhotoImage(file=os.path.join(self.icon_path,'check_icon.png'))
			self.trash_icon =  tk.PhotoImage(file=os.path.join(self.icon_path,'delete_icon.png'))
			self.dustbin_icon = tk.PhotoImage(file=os.path.join(self.icon_path,'dustbin.png'))
			self.car_icon = tk.PhotoImage(file=os.path.join(self.icon_path,'car_add.png'))
	...
	def build_menu(self):
		...
		self.menu = tk.Menu(self.cont,tearoff=0,background=col) 
		self.menu.add_command(label='Custom filter ..',state=tk.DISABLED,foreground="darkgrey")
		self.menu.add_separator()
		self.menu.add_command(label='Keep all', command = lambda: self.select_all(mode='select'),foreground="black")
		self.menu.add_command(label='Trash all', command = lambda: self.select_all(mode='trash'),foreground="black")
	...
	def build_widgets(self):
		...
		for n,column in enumerate(self.source_data_columns):
			n = n*2
			var_ = tk.StringVar() 
			...
			if self.platform == 'MAC':
				tree_data.bind("<Button-2>", self.cast_menu)
			else:
				tree_data.bind("<Button-3>", self.cast_menu)
	...
	def add_data_to_trees(self, data = None, column= None, new_separator = False):
		...
		if new_separator:
			...
			return
		if data is None:
			for column in self.source_data_columns:
	...
	def get_unique_vals(self,column,combo_sep):
		...
		if self.source_data[column].dtype in [np.float64,np.int64]:
			dat_ = self.source_data[column].astype(str)
		else:
			dat_ = self.source_data[column]
	...
	def filter_and_close(self, mode = ''):
		...
		for column,tree_ in self.trees.items():
			...
			if operator == 'AND':
				...
				if self.annotate_category.get():
					...
					collect_subframes[column] = self.source_data[column].astype(str).apply(lambda row: self.custom_search_annotate(row,keep_))
			
				else:
					try:
						collect_subframes[column] = self.source_data[column].astype(str).str.contains('|'.join(keep_))
					except:
						collect_subframes[column] = self.source_data[column].astype(str).apply(lambda row: self.custom_search(row,keep_))
		...
		if operator == 'OR' and self.annotate_category.get() == False:
			data_ = self.source_data[collect_subframes.sum(axis=1) > 0]
			
		elif self.annotate_category.get():

			collect_subframes = collect_subframes.dropna(how='all')
			data_ = pd.DataFrame()
			if len(self.source_data_columns) == 1:
			
				data_ = collect_subframes
			elif len(self.source_data_columns) > 1:
				data_ = collect_subframes[self.source_data_columns].apply(lambda row: self.join_strings(row), axis=1)
				
			elif len(self.source_data_columns) == 2:
				
				data_.loc[:,'comb_'] = collect_subframes[self.source_data_columns[0]].str.cat(collect_subframes[self.source_data_columns[1]],sep=';',na_rep ='-')
			else:
			
				collect_cols = []	
				for col in self.source_data_columns:
					if col == self.source_data_columns[0]:
						pass
					else:
						collect_cols.append(list(collect_subframes[col]))
				
				data_.loc[:,'comb_'] = collect_subframes[self.source_data_columns[0]].str.cat(collect_cols,sep=';',na_rep ='-')				
			
				
		if len(data_.index) == len(self.source_data.index) and self.mode != 'annotate' and self.annotate_category.get() == False:
