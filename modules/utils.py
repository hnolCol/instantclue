...
styleDict = {'tearoff':0,'background' : col_menu,'foreground':col_foreground}
...
LIMIT_WIDTH_NORMAL = 1550
LIMIT_HEIGHT_NORMAL = 1200

LIMIT_WIDTH_LARGE = 1800
LIMIT_HEIGHT_LARGE = 1400
...
def check_resolution_for_icons(new_width,new_height,old_width= None,old_height = None, init_window = False):
    ...
    if init_window:
        ...
        if (new_width < LIMIT_WIDTH_SMALL and old_width > LIMIT_WIDTH_SMALL) or (new_height < LIMIT_HEIGHT_SMALL and old_height > LIMIT_HEIGHT_SMALL):
            return 'NORM'
        elif (new_width < LIMIT_WIDTH_NORMAL and old_width > LIMIT_WIDTH_NORMAL) or \
        (new_width > LIMIT_WIDTH_SMALL and old_width < LIMIT_WIDTH_SMALL and \
        new_width < LIMIT_WIDTH_LARGE) or (new_height < LIMIT_HEIGHT_NORMAL and\
         old_height > LIMIT_HEIGHT_NORMAL) or  \
         (new_height > LIMIT_HEIGHT_SMALL and old_height < LIMIT_HEIGHT_SMALL and \
         new_height < LIMIT_HEIGHT_LARGE):
