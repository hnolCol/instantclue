...
class hierarchichalClustermapPlotter(object):
	...
	def __init__(self,data,Plotter,figure,
		...
		#check if numerical column > 1 - if so: clustering can be performed
		if len(self.numericColumns) == 1:
			if self.circulizeDendrogram:
				warn = WarningMessage(infoText="For circulized dendograms, at least 2 numerical columns are required.")
				warn.exec_()
				self.plotter.clean_up_figure()
				return
			else:
				#set clustering to None
				self.metricRow = "None"
		...
		if not self.get_data():
			warn = WarningMessage(infoText="Removing NaN from the data resulted in an empty data frame. You can turn of the clustering of rows and columns to display NaN values (lightgrey).")
			warn.exec_()
			self.plotter.clean_up_figure()
			return
	...
	def get_data(self):
		...
		if self.plotCorrMatrix:
			self.df = self.data.corr(method = self.corrMethod).dropna()
		elif self.metricRow == 'None' and self.metricColumn == 'None':
			self.df = self.data 
		else:
			self.df = self.data.dropna(subset = self.numericColumns)
			if self.df.empty:
				return False
	...
	def create_cluster_map(self,specificAxis=None,figure=None, redraw = True):
		'''
		'''
		if len(self.df.index) < 2 and (self.metricColumn != "None" or self.metricRow != "None"):
		#	self.progressClass.close()
		#	tk.messagebox.showinfo('Erro ..','Filtered data frame has less than 2 rows.')
			warn = WarningMessage(infoText="Filtered data frame has less than 2 rows.")
			warn.exec_()
			return
		
		#gressClass.update_progressbar_and_label(2,'Starting ...')
	...
	def update_limits(self):
		...
		if hasattr(self, 'meshKwargs'):
			...
			hclustLimit = self.plotter.get_hclust_limit_type()
			
			if isinstance(hclustLimit,str):
				if hclustLimit == 'min = -1, max = 1':
					
					self.meshKwargs['vmin'] = -1
					self.meshKwargs['vmax'] = 1
			
				elif hclustLimit == 'Center 0':
					
					max = np.nanmax(np.abs(self.df[self.numericColumns].values))
					self.meshKwargs['vmin'] = -max
					self.meshKwargs['vmax'] = max
				
				elif hclustLimit == 'Raw data':
					
					self.meshKwargs['vmin'] = np.nanmin(self.df[self.numericColumns].values)
					self.meshKwargs['vmax'] = np.nanmax(self.df[self.numericColumns].values)										
									
			elif isinstance(hclustLimit,list):
	...
	def cluster_data(self, dataFrame, metric, method):
		...
		try:
			if  metric ==  'euclidean':   
				...
				warn = WarningMessage(infoText="Data could not be clustered. This might be due to rows that contain exactly the same values.")
				warn.exec_()
				return None, None
	...
	def add_some_bindings(self,ax):
		...
		if self.circulizeDendrogram == False and not self.plotCorrMatrix:
			self.figure.canvas.mpl_connect('motion_notify_event',self.on_hover)

	def on_hover(self,event=None):
		""
		#if idxData is None - Quick Select is reset
		self.idxData = None
		if event.inaxes == self.axClusterMap:
			self.idxData = self.df.index[int(event.ydata)]
		self.plotter.sendDataIdxToQuickSelect(self.idxData)
