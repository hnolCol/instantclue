...
from .scatter_plotter import scatterPlot    			
from ...utils import INSTANT_CLUE_BLUE
class binnedScatter(object):
	...
	'''
	def __init__(self,plotter,data,numericColumns, categoricalColumns):
		'''
		'''
...
class scatterWithCategories(object):
	'''
	'''
	def __init__(self,plotter,data,figure,categoricalColumns=[], dataID = None,
		...
		self.figure = figure
		# need to set current figure by figure.number?
	...
	def add_labels_to_figure(self):
		...
		kwargs_rectangle_main = dict(edgecolor='black',clip_on=False,linewidth=0.1,fill=True, facecolor = INSTANT_CLUE_BLUE)
