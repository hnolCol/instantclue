...
from .plots.hierarchical_clustering import hierarchichalClustermapPlotter
from .plots.time_series_helper import _timeSeriesHelper
from .plots.scatter_with_categories import scatterWithCategories, binnedScatter
from .plots.grid_search_results import gridSearchVisualization 
from .plots.stacked_area_plotter import stackedAreaPlot
from .plots.line_plot import linePlotHelper
#from modules.dialogs import curve_fitting
from .plots.dim_reduction import dimensionalReductionPlot
from .plots.grid_search_results import chartToolTip
from .plots.axis_styler import axisStyler
from .plots.cluster_plotter import clusterPlotting
#from .plots.scatter_annotations import annotateScatterPoints
from .plots.scatter_plotter import scatterPlot    			
...
class Plotter(object):
	...
	def __init__(self,parent,sourceDataClass,figure):
		...
		self.set_scatter_point_properties(self.dfClass.colorManager.nanColor,self.dfClass.colorManager.alpha,50)

		self.parent = parent

		self.hoverSelectRadius = 0.05
          								
	def set_hover_select_r(self,r):
		""
		self.hoverSelectRadius = r 

	def setShowOutliers(self,showOutliers):
		""
		self.showOutliers = showOutliers

	def getShowOutlier(self):
		""
		return self.showOutliers

	def setDodge(self,dodging):
		""
		setattr(self,"dodge",dodging)
	
	def getDodge(self):
		""
		return self.dodge

	def setErrorBar(self, errorBar = 95):
		""
		self.errorBar = errorBar
	
	def getErrorBar(self):
		""
		return self.errorBar

	def get_hover_select_r(self):
		""
		return self.hoverSelectRadius
	...
	def disconnect_tooltips(self):		
		...
		for scatterPlot in scatterPlots.values():
			scatterPlot.disconnect_tooltip()
				...
				#maybe rather transmit a column dict with props instead of columsn and ID sep?
	def initiateChart(self,dataProps,selectedPlotType, colorMap, 
							specificAxis= None, redraw = False):
		'''
		intiating a chart
		'''
		try:
			numericColumns, categoricalColumns = dataProps["numericColumns"], dataProps["categoricalColumns"]
			...
			if len(categoricalColumns) == 0:
				...
				self.nonCategoricalPlotter = nonCategoricalPlotter(self,self.figure, dataProps,
																	selectedPlotType,colorMap, 
																	specificAxis)
			else:
				
				self.categoricalPlotter = categoricalPlotter(self,self.figure, dataProps,
													selectedPlotType,colorMap, 
													specificAxis)
	...
	def sendDataIdxToQuickSelect(self,dataIndex):
		""
		self.parent.hightQuickSelect(dataIndex)

	def sendSelectEventToQuickSelect(self,dataIndex):
		""
		self.parent.checkDataInQuickSelect(dataIndex)
	@property
	...
	def update_scatter_point_properties(self):
	...
	def annotate_selectedData(self,calledFromScat,dataIndex):
		'''
		'''
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			if scatterPlot != calledFromScat:
				scatterPlot.set_hover_data(dataIndex)		
	...
	def set_hover_points_invisible(self, calledFromScat = None, resetIdx = False):
		'''
		'''
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			if calledFromScat is not None and scatterPlot != calledFromScat:
				scatterPlot.set_hoverPoints_invisible(resetIdx = resetIdx)
			else:
				scatterPlot.set_hoverPoints_invisible(resetIdx = resetIdx)

	def set_resize_trigger(self,resized = True):
		""
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			scatterPlot.setResizeTrigger(resized)

	def update_background_in_scatter_plots(self, redraw = False):
		'''
		'''
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			scatterPlot.update_background(redraw=redraw)			
	...
	def set_dist_settings(self,bool = False):
		...
		if self.plotCumulativeDist != bool:
			...
			self.initiateChart(*self.plotProperties[self.plotCount])
...
#		plt.rc('ytick', labelsize= 8, direction = 'in')
	...
	def fill_axes_with_plot(self, data, x , y , hue, ax, cmap, plot_type = 'boxplot', error = 0.95,
		...
		#try:
			if plot_type == 'boxplot':
							sns.boxplot(x= x, y=y,data=data, hue = hue, palette = cmap, order = order,
								fliersize = 3, linewidth=0.65, ax = ax, hue_order = hue_order, showfliers = showOutliers)
						...
						if plot_type == 'add_swarm':
							...
							sns.pointplot(x= x, y=y, hue = hue, data=data, order = order, capsize=0.05,
										errwidth=0.4, scale=0.55,palette=cmap,ax=ax,
										hue_order = hue_order,n_boot = 500, ci = error, dodge = 0.0 if not dodge else 0.1)
			elif plot_type == 'violinplot':
							
							sns.violinplot(x= x, y=y, hue = hue, data=data, palette = cmap ,order=order,
											linewidth=0.65, ax = ax, hue_order = hue_order)
							self.give_violins_edge_color(ax)
	...
	def give_violins_edge_color(self,ax):
...
class categoricalPlotter(object):
	...
	def __init__(self,plotter, figure, dataProps, 
						selectedPlotType,colorMap, specificAxis = None):
	
		#selectedPlotType = 'stacked_area'
		self.numericColumns = dataProps["numericColumns"]
		self.categoricalColumns = dataProps["categoricalColumns"]
		self.numbNumericColumns = len(self.numericColumns)
		self.numbCategoricalColumns = len(self.categoricalColumns)
		self.dataID = dataProps["funcProps"]["kwargs"]["dataID"]
	...
	def get_data(self,dataProps):
		""
		dataProps["funcProps"]["kwargs"]["activePlotter"] = self
		dataProps["requestData"](dataProps["funcProps"])
...
#		
	...
	def outsideThreadPlotting(self):
		""
		for scatterPlot in self.get_scatter_plots().values():
			scatterPlot.outsideThreadPlotting()
	...
	def filling_axis(self,plotType,onlySelectedAxis = None, axisExport = None, forceLegend = False):
		...
		if plotType == 'countplot':
			...
			if self.plotter.splitCategories == False:				
				for n,numColumn in enumerate(self.numericColumns):
					...
					self.plotter.fill_axes_with_plot(ax=ax, x='Column',y='Value',hue = None,
									plot_type = plotType,cmap=self.colorMap,data=dataForPlotting,
									order = complColumns, error = self.error, 
									showOutliers = self.plotter.getShowOutlier(), 
									dodge = self.plotter.getDodge())
				...
				if self.numbNumericColumns != 1:
					if False:#line plot for feature implementation
						self.plotter.fill_axes_with_plot(ax=ax,x = self.numericColumns[0], y = self.numericColumns[1], 
										hue = self.categoricalColumns[0],
										plot_type = plotType, order = self.firstCategoryLevels,
										data = self.data, cmap = self.colorMap, dodge=self.plotter.getDodge(),
										error = self.error, showOutliers = self.plotter.showOutliers)								
					else:
						self.plotter.fill_axes_with_plot(ax=ax,x='Columns',y='Value',hue = self.categoricalColumns[0],
									plot_type = plotType,cmap=self.colorMap,data=sourcePlotData,
									order = self.numericColumns,dodge = self.plotter.getDodge(), showOutliers = self.plotter.showOutliers,
					...
					if False: #line plot for feature implementation
						...
						self.plotter.fill_axes_with_plot(ax=ax,x = self.categoricalColumns[0], y = self.numericColumns[0], hue = None,
										plot_type = plotType, order = self.firstCategoryLevels,
										data = self.data, cmap = self.colorMap, dodge=self.plotter.getDodge(),
										error = self.error, showOutliers = self.plotter.showOutliers)
					addLeg = False
				...
				for n,numColumn in enumerate(self.numericColumns):
					...
					self.plotter.fill_axes_with_plot(ax = ax,x= self.categoricalColumns[0], y = numColumn, 
				...
				for numColumn in self.numericColumns:
					...
					for name,group in groupedData:
						...
						self.plotter.fill_axes_with_plot(ax = ax,x= self.categoricalColumns[0], 
											y = numColumn, hue = self.categoricalColumns[1],
											data = group, order = xAxisOrder, hue_order = hueOrder,
											plot_type = plotType, cmap = self.colorMap,
											inmutableCollections = self.inmutableCollections,
											error = self.error,
											addSwarmSettings =  self.plotter.swarmSetting, showOutliers = self.plotter.showOutliers)
		...
		#self.aggregateMethod = 'count'		
				...
				self.scatterWithCategories = scatterWithCategories(
							plotter = self.plotter,
							data = self.data,
							figure = self.figure, 
							categoricalColumns = self.categoricalColumns,
							numericalColumns = self.numericColumns,
							colorMap = self.colorMap, 
							dataID = self.dataID)
	...
	def add_tooltip_to_hclust(self, annotationColumns):
		""
	...
	def disconnect_event_bindings(self):
		...
		if hasattr(self,'scatterWithCategories'):
			self.scatterWithCategories.disconnet_events()
...
class nonCategoricalPlotter(object):
	...
	def __init__(self, parent, figure, dataProps, selectedPlotType,
						colorMap, specificAxis = None, categoricalColumns = []):
		self.plotter = parent #plotter object
		self.figure = figure
		self.numericColumns = dataProps["numericColumns"]
	...
	def get_data(self,dataProps):
		...
		if self.currentPlotType == 'PCA':
			...
			#dataProps["funcProps"]["kwargs"]["activePlotter"] = self
			dataProps["requestData"](dataProps["funcProps"])
			#self.data = self.dfClass.getDataByCol"umnNames(columnList = self.numericColumns)
	...
	def add_axis_to_figure(self):
			...
			if self.currentPlotType in nonScatterPlotTypes:
				...
				self.figure.subplots_adjust(wspace=0.15, hspace = 0.15,bottom=0.15,right=0.95,top=0.95)  
	...
	def add_tooltip_to_scatter(self, toolTipData):
		'''
		'''
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			scatterPlot.add_tooltip(toolTipData)		
	...
	def change_size_by_categorical_column(self, colorProps, specificAxis = None, update = True):
		'''
		...
		for scatterPlot in scatterPlots.values():
			scatterPlot.change_size_by_categorical_column(**colorProps, specificAxis = specificAxis,
															update=update)
															
		self.plotter.updatingAxis = False
		
	def ignoreEvents(self, ignoreEvents = True):
		""
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			scatterPlot.ignoreEvents = ignoreEvents
	...
	def change_color_by_categorical_columns(self,specificAxis = None, 
		...
		scatterPlots = self.get_scatter_plots()
		for scatterPlot in scatterPlots.values():
			scatterPlot.change_color_by_categorical_columns(**kwargs,
...
# 		axisStyler(ax,forceLegend=True,kwsLegend={'leg_title':legendTitle})
	...
	def filling_axis(self,onlySelectedAxis = None, axisExport = None,plotType = None):
		...
		if plotType in ['boxplot','barplot','violinplot','swarm','add_swarm','pointplot']:
			...
			self.plotter.fill_axes_with_plot(ax=ax,x=None,y=None,hue = None,plot_type = plotType,
								cmap=colorMap,data=dataInput,order=self.numericColumns,
								inmutableCollections = self.inmutableCollections,
								error = self.error, addSwarmSettings =  self.plotter.swarmSetting, 
								showOutliers = self.plotter.showOutliers)
			...
			self._hclustPlotter = hierarchichalClustermapPlotter(self.data,
	...
	def disconnect_event_bindings(self):
		...
		for scatterPlot in self.get_scatter_plots().values():
			scatterPlot.disconnect_event_bindings()
